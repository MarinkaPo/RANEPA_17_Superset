...
if chosen_menu =="1. Задача и актуальность":
    ...
    st.markdown('''<h2 style='text-align: center; color: black;'
                >Этапы работы:</h2>''', unsafe_allow_html=True)
    st.image(os.path.join(IMG_PATH, 'superset_pipeline.png'), use_column_width='auto')

    pipeline_bar = st.expander("Подробнее об этапах работы")    
    pipeline_bar.markdown(
        """
        \n *(зелёным обозначены блоки, работа с которыми доступна студенту, красным - блоки, доступные для корректировки сотрудникам ЛИА)*
        \n**1. Блок теории:**
    ...
    st.markdown('''<h3 style='text-align: left; color: #000000;'>
                Выполните следующие задания, используя интерактивный дашборд в Superset:
                </h3>''', unsafe_allow_html=True)
    st.write('''
            \n##### 1. Ознакомьтесь с построенной вами общей таблицей данных по демографии Российской Федерации.
            \n**а).** Какие демографические показатели учтены в ней? За какие годы? 
            \n**б).** Отсортируйте таблицу по показателю естественного прироста/убыли. Какой округ показывает худшие показатели и за какой год? Соответствует ли этот регион худшему коэффициенту естественного прироста? Чем отличается показатель абсолютного прироста/убыли от коэффициента прироста/убыли?
            \n**в).** Какой субъект Российской Федерации лидирует по коэффициенту рождаемости? В каком году? Превышала ли рождаемость смертность в этом регионе?
            \n**г).** У какого субъекта Российской Федерации худшие показатели по смертности? В каком году?
            \n**д).** Какие выводы можно сделать после анализа таблицы по пунктам в) и д)?
            \n##### 2. Ознакомьтесь с построенным вами графиком Big Number.
            \n**а).** Применяя общий фильтр дашборда, посмотрите среднюю численность постоянного населения по годам во всей Российской Федерации и в городах федерального значения. Какие выводы можно сделать?
            \n##### 3. Ознакомьтесь с построенным вами графиком Pie Chart.
            \n**а).** Все ли федеральные округи там учтены? Какие округи приблизительно схожи по численности? Какой федеральный округ самый малочисленный?
            \n**б).** Что можно сказать о динамике средней численности населения федеральных округов Российской Федерации за 2015-2018 года (используя общий фильтр дашборда)?
            \n**в).** Попробуйте сделать графики со следующими фильтрами через **"CUSTOM SQL"** запрос и проанализировать полученные Pie Charts:''')
    col1,col2 = st.columns([1,1])
    col1.write('* Включаем в данные только области Российской Федерации')
    col1.write('* Включаем в данные только города федерального значения')
    col2.code("\nТерритория LIKE '%область%'")
    col2.code("\nТерритория LIKE '%город %'")
    st.write('''\n##### 4. Ознакомьтесь с построенным вами графиком Bar Chart.
            \n**а).** Меняя общий фильтр дашборда, проанализируйте динамику количества родившихся и умерших по федеральным округам.
            \n**б).** В каком федеральном округе количество родившихся превышает количество умерших людей? В каком году? 
            \n**в).** В каком федеральном округе худшая динамика по смертности? Лучшая динамика по рождаемости? Какие причины этого вы можете назвать?
            \n##### 5. Ознакомьтесь с построенным вами графиком Time-series Line Chart.
            \n**а).** Отследите общую тенденцию по годам. Какие выводы можно сделать? Какие причины такой ситуации вы можете назвать?
            \n**б).** Попробуйте построить и другие графики, используя для этого другие метрики, указанные ниже. Проанализируйте полученные графики:''')
    col1,col2 = st.columns([1,1])
    col1.write('')
    col1.write('* График миграционного прироста/убыли по городам федерального значения')
    col1.write('')
    col1.write('* График младенческой смертности (на 1000 живых родившихся детей) по федеральным округам')
    col2.code("AVG(Миграц прирост/убыль)\nТерритория LIKE '%город %'")
    col2.code("AVG(Млад смертн/1000 родившихся жив)\nТерритория LIKE '%округ%'")
    st.write('''\n##### 6. Сделайте общий вывод о демографической ситуации в Российской Федерации за 2015-2018 года.''')
